...
def get_speeding_exceptions(api, device_id, from_date_str, to_date_str):
    """Fetches speeding exceptions for a specific device and date range."""
    if not api or not device_id:
        print("ERROR get_speeding_exceptions: Missing api client or device_id.")
        return pd.DataFrame()
    try:
        # Format dates for Geotab API
        from_date_iso = f"{from_date_str}T00:00:00.000Z" # Assume UTC or handle timezone appropriately
        to_date_iso = f"{to_date_str}T23:59:59.999Z"
    
        search_criteria = {
            'fromDate': from_date_iso,
            'toDate': to_date_iso,
            'deviceSearch': {'id': device_id}, # Filter by specific device
            'ruleSearch': {'id': 'RulePostedSpeedingId'} # Geotab default speeding rule ID
        }
        print(f"DEBUG get_speeding_exceptions: Searching Geotab for device '{device_id}' from {from_date_iso} to {to_date_iso}")
        speeding_exceptions = api.get('ExceptionEvent', search=search_criteria)
        print(f"DEBUG get_speeding_exceptions: Found {len(speeding_exceptions)} raw speeding exception events.")
    
        exceptions_data = [
            {
                'Device ID': ex.get('device', {}).get('id'),
                'Event Start': ex.get('activeFrom'), # Keep original ISO format for next query
                'Event End': ex.get('activeTo'),     # Keep original ISO format
                'Duration': ex.get('duration'),
                'Rule Name': ex.get('rule', {}).get('name', 'N/A') # Get rule name
                # Add other fields if needed, like distance, diagnostic info
            }
            for ex in speeding_exceptions if ex.get('activeFrom') and ex.get('activeTo') # Ensure timestamps exist
        ]
        exceptions_df = pd.DataFrame(exceptions_data)
        # Filter out blank durations if necessary (Geotab duration might be ISO format string)
        exceptions_df = exceptions_df[exceptions_df['Duration'].notna()]
        print(f"DEBUG get_speeding_exceptions: Created DataFrame with {len(exceptions_df)} valid speeding exceptions.")
        return exceptions_df
    except Exception as e:
        print(f"ERROR get_speeding_exceptions: Failed - {e}")
        traceback.print_exc()
        return pd.DataFrame()

def get_idling_exceptions(api, device_id, from_date_str, to_date_str, min_duration_seconds=300):
    """Fetches idling exceptions (lat/lon required) for a specific device and date range exceeding a minimum duration."""
    if not api or not device_id:
        print("ERROR get_idling_exceptions: Missing api client or device_id.")
        return pd.DataFrame() # Return empty DataFrame

    try:
        from_date_iso = f"{from_date_str}T00:00:00.000Z"
        to_date_iso = f"{to_date_str}T23:59:59.999Z"
        ...
        search_criteria = {
            ...
            'ruleSearch': {'id': 'RuleIdlingId'} # Geotab default idling rule ID
        }
        print(f"DEBUG get_idling_exceptions: Searching Geotab for device '{device_id}' from {from_date_iso} to {to_date_iso}")
        idling_exceptions = api.get('ExceptionEvent', search=search_criteria)
        print(f"DEBUG get_idling_exceptions: Found {len(idling_exceptions)} raw idling exception events.")

        exceptions_data = []

        for ex in idling_exceptions:
            # --- Safely get required fields using LOWERCASE keys from Geotab ---
            lat_val = ex.get('latitude')  # Use lowercase key
            lon_val = ex.get('longitude') # Use lowercase key
            duration_obj = ex.get('duration') # Get whatever type Geotab returns
            active_from = ex.get('activeFrom')
            active_to = ex.get('activeTo')

            # --- Check if essential location data exists AND IS NUMERIC---
            # Use pd.notna and check type
            valid_lat = pd.notna(lat_val) and isinstance(lat_val, (int, float))
            valid_lon = pd.notna(lon_val) and isinstance(lon_val, (int, float))

            if valid_lat and valid_lon and active_from and active_to:
                # --- Duration Handling (More Robust) ---
                duration_seconds = -1 # Default if calculation fails
                duration_display = str(duration_obj) # Default display is string representation
                try:
                    # Try parsing ISO timestamps (common Geotab format)
                    start_dt = pd.to_datetime(active_from, errors='coerce', utc=True)
                    end_dt = pd.to_datetime(active_to, errors='coerce', utc=True)
                    if pd.notna(start_dt) and pd.notna(end_dt):
                        delta = end_dt - start_dt
                        duration_seconds = delta.total_seconds()
                        # Format duration_display more nicely here (e.g., MM:SS)
                        if duration_seconds >= 0:
                             minutes, seconds = divmod(int(duration_seconds), 60)
                             duration_display = f"{minutes:02d}:{seconds:02d}"
                        else:
                             duration_display = "Invalid" # Handle negative delta?
                    else: # If timestamps are invalid, try parsing duration_obj IF it's an ISO string
                        if isinstance(duration_obj, str) and duration_obj.startswith("PT"):
                             # Add proper ISO8601 duration parsing here if needed (e.g., using isodate library)
                             # For now, just keep original string if timestamps fail
                             duration_display = duration_obj
                        # else: keep str(duration_obj) from above
                except Exception as dur_err:
                    print(f"WARN get_idling_exceptions: Could not calculate/format duration from '{duration_obj}' or timestamps {active_from}/{active_to}: {dur_err}")
                    duration_display = str(duration_obj) # Keep original on error

                # --- Check if calculated duration meets minimum ---
                if duration_seconds >= min_duration_seconds:
                    exceptions_data.append({
                        'Device ID': ex.get('device', {}).get('id'),
                        'Event Start': active_from,
                        'Event End': active_to,
                        # Use consistent LOWERCASE keys for the DataFrame columns
                        'duration': duration_display,
                        'calculated_seconds': duration_seconds,
                        'latitude': lat_val,  # Use lowercase key
                        'longitude': lon_val  # Use lowercase key
                    })
                # else: print(f"DEBUG get_idling_exceptions: Skipping idling event, duration {duration_seconds}s < {min_duration_seconds}s")
            # else: print(f"DEBUG get_idling_exceptions: Skipping event due to missing/invalid lat/lon/timestamps. Lat: {lat_val}, Lon: {lon_val}")

        # Create DataFrame from valid collected data
        if not exceptions_data: # Handle case where no valid exceptions were found
             print("INFO get_idling_exceptions: No valid idling exceptions found meeting criteria.")
             return pd.DataFrame() # Return empty

        exceptions_df = pd.DataFrame(exceptions_data)

        # dropna should no longer be needed or cause KeyError because we pre-validated lat/lon
        # If you still want it, use lowercase: exceptions_df.dropna(subset=['latitude', 'longitude'], inplace=True)

        print(f"DEBUG get_idling_exceptions: Created DataFrame with {len(exceptions_df)} valid idling exceptions over {min_duration_seconds}s.")
        return exceptions_df

    except Exception as e:
        print(f"ERROR get_idling_exceptions: Failed - {e}")
        traceback.print_exc()
        return pd.DataFrame() # Return empty DataFrame on error


def get_speeding_locations(api, speeding_exceptions_df):
    """Finds the location where max speed occurred during each speeding event."""
    if not api: 
        return pd.DataFrame()
    if speeding_exceptions_df.empty: 
        return pd.DataFrame()
    
    all_max_speed_records = []
    print(f"DEBUG get_speeding_locations: Getting max speed points for {len(speeding_exceptions_df)} exceptions...")
    
    for index, row in speeding_exceptions_df.iterrows():
        device_id = row['Device ID']
        event_start = row['Event Start']
        event_end = row['Event End']
    
        # Check for valid timestamps before querying
        if not event_start or not event_end:
            print(f"WARN get_speeding_locations: Skipping exception due to missing start/end time for device {device_id}")
            continue
    
        try:
            search_criteria = {
                'deviceSearch': {'id': device_id},
                'fromDate': event_start,
                'toDate': event_end
            }
            # Fetch log records within the exception event time window
            log_records = api.get('LogRecord', search=search_criteria)
    
            if log_records:
                # Find the record with the maximum speed
                selected_record = max(log_records, key=lambda record: record.get('speed', 0))
                speed_kph = selected_record.get('speed', 0)
                speed_mph = round(speed_kph * 0.621371)
    
                all_max_speed_records.append({
                    'lat': selected_record.get('latitude'),
                    'lon': selected_record.get('longitude'),
                    'max_speed_mph': speed_mph,
                    'timestamp': selected_record.get('dateTime'),
                    'event_duration': row['Duration'], # Keep original duration format
                    # Add other relevant info from the exception row if needed
                    'device_id': device_id
                })
        except Exception as log_err:
             print(f"ERROR get_speeding_locations: Failed getting log records for device {device_id}, event {event_start}: {log_err}")
             # Continue to next exception
    
    locations_df = pd.DataFrame(all_max_speed_records)
    # Ensure essential columns exist and drop rows with missing lat/lon
    if not locations_df.empty:
         locations_df.dropna(subset=['lat', 'lon'], inplace=True)
    print(f"DEBUG get_speeding_locations: Found {len(locations_df)} max speed locations.")
    return locations_df
