...
print("Value for GEOTAB_USERNAME_SECRET:",
      os.environ.get('GEOTAB_USERNAME_SECRET'))
...
try:
    ...
    if not gspread_client or not drive_service:
        raise ConnectionError(
            "Google clients (GSpread/Drive) failed to initialize.")
    ...
    print(
        f"FATAL: Error during application startup client initialization: {startup_error}"
    )
...
def get_depot_from_ras(ras_df):
    ...
    if not yard_col:
        print(
            "WARNING: Cannot determine depot, missing Yard column in RAS data."
        )
        return None
    try:
        ...
        print(
            "WARNING: Could not access first row of RAS DataFrame for depot.")
        return None
    ...
    for depot_name in config.DEPOT_LOCS.keys():
        if depot_name.lower() in yard_string_lower:
            ...
            return depot_name
    print(
        f"WARNING: Could not match yard string '{yard_string}' to known depots in config."
    )
    return None
...
def get_map_data():
    ...
    if not all([geotab_client, gspread_client, drive_service, mapbox_token]):
        ...
        return jsonify({
            "error":
            "Server configuration error: required clients not ready."
        }), 503
    ...
    try:
        ...
        try:
            date_obj = datetime.datetime.strptime(date_str_ymd,
                                                  '%Y-%m-%d').date()
        except ValueError:
        ...
        if date_obj >= current_monday:
            ...
            try:
                ...
                print(
                    f"INFO: Calling get_current_ras_data with date string: {date_str_for_current_ras}"
                )  # Debug
                ...
                if result_tuple and len(result_tuple) == 3:
                    ...
                    if am_routes_to_buses is None:
                        am_routes_to_buses = {}  # Ensure dict type
                    if pm_routes_to_buses is None:
                        pm_routes_to_buses = {}  # Ensure dict type
                else:
                    print(
                        "WARNING: get_current_ras_data did not return expected tuple."
                    )
            ...
            try:
                print(
                    f"INFO: Calling get_historical_ras_data with date object: {date_obj}"
                )  # Debug
                ...
                if result_tuple and len(result_tuple) == 3:
                    ...
                    print(
                        "WARNING: get_historical_ras_data did not return expected tuple."
                    )
                    # Keep defaults
        ...
        if rasdf is None:
            return jsonify(
                {"error":
                 "Failed to retrieve RAS data due to server error."}), 500
        depot = None
        if not rasdf.empty:
            ...
            if not am_routes_to_buses and not pm_routes_to_buses:
                print(
                    f"WARNING: Route {route_input} found in RAS for {date_str_ymd}, but no AM/PM vehicles assigned."
                )
        else:
            print(
                "WARNING: No RAS data found for route/date. Cannot determine depot or vehicles."
            )
            return jsonify({
                "am_map": "<div>No Scheduling (RAS) data found.</div>",
                "pm_map": "<div>No Scheduling (RAS) data found.</div>",
                "dvi_link": "#",
                "opt_data": [],
                "vehilce_am" : None,
                "vehicle_pm" : None
        ...
        if drive_service and depot:
            print(
                f"Attempting to find DVI file for Depot: {depot}, Route: {route_input}, Date: {date_str_ymd}"
            )
            file_info = data_sources.find_drive_file(drive_service,
                                                     config.ROOT_FOLDER_ID,
                                                     depot, date_str_ymd,
                                                     route_input,
                                                     config.DRIVE_ID)
            if file_info and isinstance(file_info,
                                        dict) and 'webViewLink' in file_info:
            ...
            print(
                "INFO: Skipping DVI file search (Drive service not ready or depot unknown)."
            )
        # --- End Find DVI Link ---

        # 3. Fetch OPT Dump Data
        print("Fetching OPT Dump data...")
        optdf = data_sources.get_opt_dump_data(auth_clients.get_db_connection,
                                               route_input, date_obj)

        if optdf is None:
            return jsonify(
                {"error": "Failed to retrieve route optimization data."}), 500
        ...
        if optdf.empty:
            ...
            try:
                print(
                    f"DEBUG: Converting {len(optdf)} OPT rows to JSON for response (Forcing String)."
                )
                ...
                for col in optdf_serializable.columns:
                    try:
                        optdf_serializable[col] = optdf_serializable[
                            col].astype(str)
                    except Exception as str_conv_err:
                        print(
                            f"WARN: Could not force column '{col}' to string: {str_conv_err}. Keeping original values which might cause issues."
                        )
                ...
                optdf_serializable = optdf_serializable.replace({
                    'nan': '',
                    'NaT': '',
                    '<NA>': ''
                }).fillna('')
                ...
                print(
                    "DEBUG: OPT DataFrame successfully converted to JSON list."
                )

            except Exception as json_err:
                print(
                    f"ERROR: Failed during revised OPT DataFrame to JSON conversion: {json_err}"
                )
                traceback.print_exc()
        ...
        if optdf is not None and not optdf.empty and optdf_json is None:
            print(
                "WARNING: Sending empty OPT data in response due to conversion error."
            )
            optdf_json = []
        # --- End JSON Conversion ---
            ...
            try:
                ...
                if am_bus_number:
                    ...
                    am_start_dt = datetime.datetime.combine(
                        date_obj, datetime.time(am_start_hour,
                                                am_start_minute))
                    am_end_dt = datetime.datetime.combine(
                        date_obj, datetime.time(am_end_hour, am_end_minute))
                    am_vehicle_data_df = data_sources.fetch_bus_data(
                        geotab_client, am_bus_number, am_start_dt, am_end_dt)
                    am_route_polyline = processing.convert_to_polyline(
                        am_vehicle_data_df)
                    am_route_data = processing.prepare_route_data(am_row, "am")
                    am_map_obj = map_plotting.plot_route_updated(
                        am_route_data, am_vehicle_data_df, am_route_polyline,
                        mapbox_token)
                    if am_map_obj:
                        am_map_html = am_map_obj._repr_html_()
                        print("AM Map generated.")
                    else:
                        print("AM Map plotting returned None.")
                else:
                    print("No AM vehicle number found.")
            except Exception as am_err:
                print(f"ERROR: AM map processing: {am_err}")
                traceback.print_exc()
        else:
            ...
            try:
                ...
                if pm_bus_number:
                    ...
                    pm_start_dt = datetime.datetime.combine(
                        date_obj, datetime.time(pm_start_hour,
                                                pm_start_minute))
                    pm_end_dt = datetime.datetime.combine(
                        date_obj, datetime.time(pm_end_hour, pm_end_minute))
                    pm_vehicle_data_df = data_sources.fetch_bus_data(
                        geotab_client, pm_bus_number, pm_start_dt, pm_end_dt)
                    pm_route_polyline = processing.convert_to_polyline(
                        pm_vehicle_data_df)
                    pm_route_data = processing.prepare_route_data(pm_row, "pm")
                    pm_map_obj = map_plotting.plot_route_updated(
                        pm_route_data, pm_vehicle_data_df, pm_route_polyline,
                        mapbox_token)
                    if pm_map_obj:
                        pm_map_html = pm_map_obj._repr_html_()
                        print("PM Map generated.")
                    else:
                        print("PM Map plotting returned None.")
                else:
                    print("No PM vehicle number found.")
            except Exception as pm_err:
                print(f"ERROR: PM map processing: {pm_err}")
                traceback.print_exc()
        else:
        ...
        print(
            f"--- Request completed in {duration.total_seconds():.2f} seconds ---"
        )

        return jsonify({
            "am_map": am_map_html
            or "<div>No AM map generated or data available.</div>",
            "pm_map": pm_map_html
            or "<div>No PM map generated or data available.</div>",
            "dvi_link": dvi_webview_link or "#",
            "opt_data":
            optdf_json,
            "vehicle_am": am_bus_number if am_bus_number else None,
            "vehicle_pm": pm_bus_number if pm_bus_number else None
        })
...
if __name__ == '__main__':
    ...
    if not all([geotab_client, gspread_client, drive_service, mapbox_token]):
        print(
            "FATAL: Cannot start Flask server - essential clients failed initialization during startup."
        )
    else:
        print("Starting Flask server...")
        app.run(debug=True, host='0.0.0.0',
                port=5000)  # Add use_reloader=False if needed for schedulers
